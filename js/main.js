...
const pageModuleDependencies = {
  ...
  dailyPage: ["daily"],
  taskSchedulerPage: ["boarding", "customer", "taskScheduler"],
  // Show rosters can include boarding dogs; direct links need their profile helpers too.
  dogShowPage: ["boarding", "customer", "dogShow"],
  ...
  // Customer profiles and file views use linked-boarding helpers immediately;
  // do not race the optional idle alert warmup on a fresh customer login.
  customerPage: ["customer", "boarding"],
  customerRequestsPage: ["customer", "boarding"],
  customerUpdatesPage: ["customer", "boarding"],
  customerFilesPage: ["customer", "boarding"],
};
...
function loadNamedPageModule(name) {
  ...
  // Scheduler setup immediately reads canonical boarding profiles. Load those
  // helpers first, including when arriving directly from a Settings screen.
  const prerequisites = name === "taskScheduler" ? ["boarding", "customer"] : [];
  const promise = Promise.all(prerequisites.map(loadNamedPageModule)).then(loader).then(() => {

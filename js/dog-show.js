...
function dogShowMasterCalendarYearHtml(items = []) {
  ...
  const months = Array.from({ length: 12 }, (_, monthIndex) => {
    ...
    const dayButtons = Array.from({ length: last.getDate() }, (_, index) => {
      ...
      return `<button type="button" class="${dateKey === todayDate() ? "is-today" : ""}${dayItems.length ? " has-shows" : ""}" data-show-calendar-jump-date="${dateKey}" data-show-calendar-jump-view="day"${showPreview ? ` data-show-preview="${escapeHtml(showPreview)}" title="${escapeHtml(showPreview)}"` : ""} aria-label="${escapeHtml(accessibleLabel)}"><span>${index + 1}</span><em>${dots}</em></button>`;
    }).join("");

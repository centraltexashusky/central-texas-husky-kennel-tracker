...
Before each release, run `node tools/stamp-release-version.mjs` and include the updated `index.html` in the release commit. The app heading displays the release timestamp as `vYYYY.MM.DD.HH` in `America/Chicago` time; it stays fixed until the next release. Push verified repairs to the production `main` branch and confirm GitHub Pages finishes publishing so they can be tested immediately.

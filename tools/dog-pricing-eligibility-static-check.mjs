...
assert.match(boarding, /function boardingStayRequestServiceCatalog[\s\S]*customerPricingScopeForDog\(record, user\)[\s\S]*serviceCatalogForStayRequests\(\)[\s\S]*serviceMatchesPricingScopeForResolution\(service, pricingScope\)/, "the staff stay editor starts from the full catalog and filters add-on services by the individual dog's pricing scope");

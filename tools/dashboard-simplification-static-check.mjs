...
for (const [label, source] of [["generated module", shared], ["legacy bundle", script]]) {
  ...
  for (const expected of ["Needs Action Today", "Exercise Due", "Training Due", "Baths Due", "Females In Heat"]) {
    if (!renderDashboard.includes(expected)) failures.push(`${label} no longer renders ${expected}.`);
  }
...
for (const removedMarkup of ["dashboardPriorityCards", "dashboardTimelineSection", 'id="dashboardTimeline"']) {

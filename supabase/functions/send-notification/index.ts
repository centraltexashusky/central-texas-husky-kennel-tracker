...
async function callerIsStaff(adminClient: ReturnType<typeof createClient>, email: string) {
  ...
  if (error) {
    console.warn("Could not resolve notification sender role.", error.message);
    return false;
  }
  return (data || []).some((row) => {
    const payload = (row?.payload && typeof row.payload === "object" ? row.payload : {}) as Record<string, unknown>;
    const role = String(payload.role || "").toLowerCase();
    const removed = payload.removed === true || String(payload.removed || "false").toLowerCase() === "true";
...
async function notificationContent(adminClient: ReturnType<typeof createClient>, eventName: string, record: Record<string, unknown>, notification: Record<string, unknown> = {}) {
  ...
  if (eventName === "dogShowResultPublished") {
    ...
    const body = [
      ...
      record.awards ? `BOB/BOV and breed awards: ${record.awards}` : "",
      record.groupAward ? `Regular Group award: ${record.groupAward}` : "",
      record.groupJudge ? `Regular Group judge: ${record.groupJudge}` : "",
      record.bisAward ? `Regular BIS award: ${record.bisAward}` : "",
      record.bisJudge ? `Regular BIS judge: ${record.bisJudge}` : "",
      record.ohGroupAward ? `OH Group award: ${record.ohGroupAward}` : "",
